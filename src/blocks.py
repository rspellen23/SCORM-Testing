...
BLOCKS = {
    ...
    "dragDrop":         _b(STABLE, True,  DROP,       "drag labels onto targets / a diagram (partial credit)"),
    "matching":         _b(STABLE, True,  DROP,       "match items to partners (partial credit)"),
    "sequence":         _b(STABLE, True,  DROP,       "put steps in the correct order (partial credit)"),
    "fillBlank":        _b(STABLE, True,  DROP,       "fill-in-the-blank text entry (partial credit, accept-list)"),
    "wordSearch":       _b(STABLE, True,  DROP,       "find hidden words in a letter grid, built from a term list (partial credit)"),
    "crossword":        _b(STABLE, True,  DROP,       "interlocking crossword built from clue/answer pairs (partial credit)"),
    "gameShow":         _b(STABLE, True,  DROP,       "spin-the-wheel review: MCQ per slice, spin to draw one (partial credit)"),
    "quizBoard":        _b(STABLE, True,  DROP,       "Jeopardy-style category board: pick a tile, answer its MCQ; escalating point values (weighted partial credit)"),
    "speedStreak":      _b(STABLE, True,  DROP,       "fast one-at-a-time MCQ run with a consecutive-correct streak and optional cosmetic timer (partial credit)"),
    "questionBank":     _b(STABLE, True,  DROP,       "randomized question pool (draws N at runtime, shuffles options)"),
    "scenario":         _b(STABLE, True,  DROP,       "decision walk-through (situation + choices + feedback; linear)"),
    "reflection":       _b(STABLE, True,  DROP,       "free-text / open-response reflection (non-graded, completion-only; self-check against a model answer + rubric)"),
